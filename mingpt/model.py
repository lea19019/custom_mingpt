...
class RotaryEmbedding(nn.Module):
    ...
    def forward(self, seq_len, device=None):
        ...
        cos = torch.cos(emb)[:, :self.dim//2] # Here I guess I would do :self.dim/2
        sin = torch.sin(emb)[:, :self.dim//2]
        # print("-------DEBUG RotaryEmbedding forward-------")
        # print("dim:", self.dim)
...
def apply_rotary_pos_emb(q, k, cos, sin, interleaved=False):
    ...
    # print("-------DEBUG apply_rotary_pos_emb-------")
...
class SwiGLU(nn.Module):
    def __init__(self, dim_in, dim_hidden=None, dim_out=None, bias=True, dropout_rate=0.1):
        super().__init__()
...
class Block(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.ln_2 = nn.RMSNorm(config.n_embd) if config.use_rmsnorm else nn.LayerNorm(config.n_embd)
        if config.use_swiglu:
            self.mlpf = SwiGLU(config.n_embd, dropout_rate=config.resid_pdrop)
        else:
...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        modules = {
            ...
            'ln_f': nn.RMSNorm(config.n_embd) if config.use_rmsnorm else nn.LayerNorm(config.n_embd),
        }
    ...
    def forward(self, idx, targets=None):
        ...
        if self.config.use_rope:
            ...
            # print('x after drop shape:', x.shape)
        else:
        ...
        for block in self.transformer.h:
            x = block(x)
        # print('x after blocks shape:', x.shape)
        
        x = self.transformer.ln_f(x)
        # print('x after ln_f shape:', x.shape)
        
        logits = self.lm_head(x)
        # print('logits shape:', logits.shape)

        # print('targets shape:', targets.shape if targets is not None else 'None')
        # print('logits view shape:', logits.view(-1, logits.size(-1)).shape if targets is not None else 'N/A')
        # print('targets view shape:', targets.view(-1).shape if targets is not None else 'N/A')
